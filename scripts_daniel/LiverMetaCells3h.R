...
rpkmdata<-read.table("../WD/LI_3h_2x_all_batch_correction_norm_express.txt", header=T)
data <- t(rpkmdata)
...
rpkmdata <- cbind(LI_HEK_3h_2x, LI_CAP_3h_2x)
data <- cbind(MetaLiver, LI_HEK_3h_2x, LI_CAP_3h_2x)
...
LiverBcell_cor <- CellsCor[1,(7:569)]
...
LiverEndothelial_cor <- CellsCor[2,(7:569)]

#LiverEndothelial.level <- cRamp.orig(LiverEndothelial_cor, colors = c("white","white","red","darkred"))
LiverEndothelial.level <- as.factor(findInterval(LiverEndothelial_cor, c(min(LiverEndothelial_cor),max(LiverEndothelial_cor)), rightmost.closed = FALSE, all.inside = FALSE))
LiverEndothelial.level <- c("white")[LiverEndothelial.level]
...
LiverHepatocyte_cor <- (CellsCor[3,(7:569)]+.1)*4
...
LiverKupffer_cor <- CellsCor[4,(7:569)]
...
LiverNKcell_cor <- CellsCor[5,(7:569)]
...
Mitochondrial_cor <- CellsCor[6,(7:569)]
...
LI_HEK_3h_2xFeat <- CellFeat[grep('^LI_HEK_3h_2x', rownames(CellFeat)),]
LI_CAP_3h_2xFeat <- CellFeat[grep('^LI_CAP_3h_2x', rownames(CellFeat)),]
CellFeat <- rbind(LI_HEK_3h_2xFeat, LI_CAP_3h_2xFeat)
...
load("../WD/CD63liver3hCellsQC_tSNE-100noid-3nod-200non_497Tabula100Genes_tSNEresults.Rdata")
...
### Plot data
pdf("Liver3hMetaCells.pdf", width=10, height=10)
par(mfrow=c(4,4), mar=c(1,1,1,1))

load("../WD/CD63liver3hCellsQC_tSNE-100noid-3nod-40non_497Tabula100Genes_tSNEresults.Rdata")
g <- tSNEresults$g
