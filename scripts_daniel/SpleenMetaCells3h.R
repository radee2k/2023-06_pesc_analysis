...
rpkmdata<-read.table("../WD/S_3h_2x_all_batch_correction_norm_express.txt", header=T)
...
rpkmdata <- cbind(S_HEK_3h_2x, S_CAP_3h_2x)
data <- cbind(MetaSpleen, S_HEK_3h_2x, S_CAP_3h_2x)
...
SpleenBcell_cor <- CellsCor[1,(5:765)]
...
SpleenMacrophage_cor <- CellsCor[2,(5:765)]

#SpleenMacrophage.level <- cRamp.orig(SpleenMacrophage_cor, colors = c("white","white","red","darkred"))
SpleenMacrophage.level <- as.factor(findInterval(SpleenMacrophage_cor, c(min(SpleenMacrophage_cor),0.05,0.1,max(SpleenMacrophage_cor)), rightmost.closed = FALSE, all.inside = FALSE))
SpleenMacrophage.level <- c("white","pink","red")[SpleenMacrophage.level] #c("white","greenyellow", "green3", "darkgreen")
...
SpleenTcell_cor <- CellsCor[3,(5:765)]
...
Mitochondrial_cor <- CellsCor[4,(5:765)]
...
S_HEK_3h_2xFeat <- CellFeat[grep('^S_HEK_3h_2x', rownames(CellFeat)),]
S_CAP_3h_2xFeat <- CellFeat[grep('^S_CAP_3h_2x', rownames(CellFeat)),]
CellFeat <- rbind(S_HEK_3h_2xFeat, S_CAP_3h_2xFeat)
...
write.csv(Numbers, file="SpleenNumbers3h.csv")
write.csv(aiNumbers, file="aiSpleenNumbers3h.csv")

###################################################################

load("../WD/CD63spleen3hCellsQC_tSNE-100noid-3nod-200non_299100TabulaGenes_tSNEresults.Rdata")
...
### Plot data
pdf("Spleen3hMetaCells.pdf", width=10, height=10)
par(mfrow=c(4,4), mar=c(1,1,1,1))

load("../WD/CD63spleen3hCellsQC_tSNE-100noid-3nod-40non_299100TabulaGenes_tSNEresults.Rdata")
g <- tSNEresults$g
