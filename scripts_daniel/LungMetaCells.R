...
rpkmdata<-read.table("../WD/Lu_30m_1x_2x_all_batch_correction_norm_express.txt", header=T)
rpkmdata<-rpkmdata[,1:946]
data <- t(rpkmdata)
...
rpkmdata <- cbind(LU_HEK_30m_1x, LU_CAP_30m_1x)
data <- cbind(MetaLung, LU_HEK_30m_1x, LU_CAP_30m_1x)
...
LungBcell_cor <- CellsCor[1,(13:481)]
...
LungCiliated_cor <- CellsCor[2,(13:481)]

#LungCiliated.level <- cRamp.orig(LungCiliated_cor, colors = c("white","white","red","darkred"))
LungCiliated.level <- as.factor(findInterval(LungCiliated_cor, c(min(LungCiliated_cor),0.05,0.1,max(LungCiliated_cor)), rightmost.closed = FALSE, all.inside = FALSE))
LungCiliated.level <- c("white","pink","red")[LungCiliated.level]
...
LungClassicMonocyte_cor <- CellsCor[3,(13:481)]

#LungClassicMonocyte.level <- cRamp.orig(LungClassicMonocyte_cor, colors = c("white","white","red","darkred"))
LungClassicMonocyte.level <- as.factor(findInterval(LungClassicMonocyte_cor, c(min(LungClassicMonocyte_cor),0.05,0.1,max(LungClassicMonocyte_cor)), rightmost.closed = FALSE, all.inside = FALSE))
LungClassicMonocyte.level <- c("white","pink","red")[LungClassicMonocyte.level]
...
LungEndothelial_cor <- CellsCor[4,(13:481)]

#LungEndothelial.level <- cRamp.orig(LungEndothelial_cor, colors = c("white","white","red","darkred"))
LungEndothelial.level <- as.factor(findInterval(LungEndothelial_cor, c(min(LungEndothelial_cor),0.05,max(LungEndothelial_cor)), rightmost.closed = FALSE, all.inside = FALSE))
LungEndothelial.level <- c("white","pink")[LungEndothelial.level]
...
LungEpithelial_cor <- CellsCor[5,(13:481)]
...
LungLeukocyte_cor <- CellsCor[6,(13:481)]

#LungLeukocyte.level <- cRamp.orig(LungLeukocyte_cor, colors = c("white","white","red","darkred"))
LungLeukocyte.level <- as.factor(findInterval(LungLeukocyte_cor, c(min(LungLeukocyte_cor),0.05,0.1,max(LungLeukocyte_cor)), rightmost.closed = FALSE, all.inside = FALSE))
LungLeukocyte.level <- c("white","pink","red")[LungLeukocyte.level]
...
LungMonocyte_cor <- CellsCor[7,(13:481)]
...
LungMyeloid_cor <- CellsCor[8,(13:481)]
...
LungNKcell_cor <- CellsCor[9,(13:481)]
...
LungStromal_cor <- CellsCor[10,(13:481)]

#LungStromal.level <- cRamp.orig(LungStromal_cor, colors = c("white","white","red","darkred"))
LungStromal.level <- as.factor(findInterval(LungStromal_cor, c(min(LungStromal_cor),0.05,max(LungStromal_cor)), rightmost.closed = FALSE, all.inside = FALSE))
LungStromal.level <- c("white","pink")[LungStromal.level]
...
LungTcell_cor <- CellsCor[11,(13:481)]
...
Mitochondrial_cor <- CellsCor[12,(13:481)]
...
LU_HEK_30m_1xFeat <- CellFeat[grep('^LU_HEK_30m_1x', rownames(CellFeat)),]
LU_CAP_30m_1xFeat <- CellFeat[grep('^LU_CAP_30m_1x', rownames(CellFeat)),]
CellFeat <- rbind(LU_HEK_30m_1xFeat, LU_CAP_30m_1xFeat)
...
rownames(aiNumbers) <- c("HEK","CAP", "Bcells", "BcellsHEK", "BcellsCAP", "Ciliateds", "CiliatedsHEK", "CiliatedsCAP", "ClassicMonocytes", "ClassicMonocytesHEK", "ClassicMonocytesCAP", "Endothelials", 
                         "EndothelialsHEK", "EndothelialsCAP", "Epithelials", "EpithelialsHEK", "EpithelialsCAP", "Leukocytes", "LeukocytesHEK", "LeukocytesCAP", "Monocytes", "MonocytesHEK", 
                         "MonocytesCAP", "Myeloids", "MyeloidsHEK", "MyeloidsCAP", "NKcells", "NKcellsHEK", "NKcellsCAP", "Stromals", "StromalsHEK", "StromalsCAP", "Tcells", "TcellsHEK", "TcellsCAP")
write.csv(Numbers, file="LungNumbers.csv")
write.csv(aiNumbers, file="aiLungNumbers.csv")

###################################################################

load("../WD/CD63lung1x30mCellsQC_tSNE-100noid-3nod-200non_983100TabulaGenes_tSNEresults.Rdata")
...
### Plot data
pdf("Lung30mMetaCells.pdf", width=10, height=10)
par(mfrow=c(4,4), mar=c(1,1,1,1))

load("../WD/CD63lung1x30mCellsQC_tSNE-100noid-3nod-20non_983100TabulaGenes_tSNEresults.Rdata")
g <- tSNEresults$g
