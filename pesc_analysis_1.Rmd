...
```{r most expressed cells, fig.width=10, cache = T}
...
```{r, fig.width= 15, fig.height=5, cache = T}
...
```{r most expressed, fig.width=10, cache = T}
...
```{r, cache = T, fig.width= 15, fig.height= 5}
...
```{r, cache = T, fig.width= 7, fig.height= 5, , eval = F}
...
### Find cell clusters 
```{r, cache = T, fig.width= 15, fig.height= 10}
ds_cf <- FindNeighbors(ds_cf, reduction = "pca", dims = 1:8, verbose = FALSE) %>%
  FindClusters(resolution = 0.7, verbose = FALSE)

p1 <- DimPlot(ds_cf, group.by = c("pat", "diag", "type"), pt.size = 1.5)


alpha_colors <- hue_pal()(12) #  number of colors equal to number of clusters - required to change the alpha

p2 <- DimPlot(ds_cf, pt.size = 2, label = T, cols = alpha(alpha_colors, .6)) + 
  labs(title = "clusters") + 
