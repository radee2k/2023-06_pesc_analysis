...
### Clustering 
```{r clustering_cells_umap, fig.height=10, cache = F, fig.width=15}
...
```{r most_diff_bar1, cache = F, fig.width=15, fig.height=10}
setDT(c_markers)
...
```{r cluster_heatmap1,  cache = F, fig.width= 15, fig.height= 15}
...
```{r cells2 DEseq - low min.pct,  message = F, error = F, warning=F}
...
Plot most differentially expressed genes
```{r most_diff_bar_2, cache = F, fig.width=15, fig.height=10}
setDT(c_markers2)
...
```{r cluster_heatmap_2,  cache = F, fig.width= 15, fig.height= 15}
...
```{r cells2 DE wilcox,  message = F, warning=F, error=F}
...
Plot most differentially expressed genes
```{r most_diff_bar_3, cache = F, fig.width=15, fig.height=10}
setDT(c_markers3)
...
```{r cluster_heatmap_3,  cache = F, fig.width= 15, fig.height= 18}
...
```{r cells2 DE mast,  message = F, warning=F, error=F}
...
```{r}
ds_cf_sep <- SCTransform(ds_cf, vst.flavor = "v2", verbose = FALSE) %>%
  RunPCA(npcs = 30, verbose = FALSE) %>%
  RunUMAP(reduction = "pca", dims = 1:30, verbose = FALSE)
  # RunUMAP(reduction = "pca", dims = 1:30, verbose = FALSE, n.components = 2)
# table(ds_cf@meta.data[c("pat", "diag", "type")])
...
### PCA
```{r dim_reduc_evs_wo_cells_FT_SP_SP_genes}
...
```{r correlation of PCs with metadata EVs per fraction_wo_cells_FT_SP_genes, cache = F, fig.width= 10}
# adapted from https://jdblischak.github.io/singlecell-qtl/pca-variable.html
get_r2 <- function(x, y) {
  stopifnot(length(x) == length(y))
  model <- lm(y ~ x)
